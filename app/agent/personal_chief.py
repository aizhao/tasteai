...
checkpointer = MongoDBSaver(client=mongodb_client)
...
def list_threads() -> list[dict]:
    """获取所有会话列表，包含预览信息"""
    threads = []
    distinct_ids = checkpointer.checkpoint_collection.distinct(
        "thread_id", {"checkpoint_ns": ""}
    )

    for tid in distinct_ids:
        checkpoint = checkpointer.get({"configurable": {"thread_id": tid}})
        if not checkpoint:
            continue
